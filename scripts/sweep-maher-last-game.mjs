#!/usr/bin/env node
/**
 * Sweep SCORE_MAHER_LAST_GAME_BLEND (extra pull toward last-1 ratings).
 * Rank primarily by ROI, then Brier.
 *
 * Usage:
 *   NEXT_PUBLIC_EXPRESS_SERVER=http://localhost:5050/ \
 *     node scripts/sweep-maher-last-game.mjs --from 2026-07-01 --to 2026-09-15
 */
...
const VARIANTS = [
  { id: "lg0", label: "LAST_GAME=0 (baseline)", env: { SCORE_MAHER_LAST_GAME_BLEND: "0" } },
  { id: "lg10", label: "LAST_GAME=0.10", env: { SCORE_MAHER_LAST_GAME_BLEND: "0.1" } },
  { id: "lg15", label: "LAST_GAME=0.15", env: { SCORE_MAHER_LAST_GAME_BLEND: "0.15" } },
  { id: "lg25", label: "LAST_GAME=0.25", env: { SCORE_MAHER_LAST_GAME_BLEND: "0.25" } },
];
...
console.log("\n\nMaher last-game blend comparison (rank by ROI, then Brier)");
console.log(
  [
    "id".padEnd(8),
    "n".padStart(6),
...
const outPath = resolve(projectRoot, "scripts/output/maher-last-game-sweep.json");
writeFileSync(
  outPath,
