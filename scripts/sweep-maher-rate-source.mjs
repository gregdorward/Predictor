#!/usr/bin/env node
/**
 * Sweep SCORE_MAHER_RATE_SOURCE (xg | npxg | goals | mix).
 * Rank primarily by ROI, then Brier.
 *
 * Usage:
 *   NEXT_PUBLIC_EXPRESS_SERVER=http://localhost:5050/ \
 *     node scripts/sweep-maher-rate-source.mjs --from 2026-07-01 --to 2026-09-15
 */
...
const VARIANTS = [
  { id: "xg", label: "RATE_SOURCE=xg (baseline)", env: { SCORE_MAHER_RATE_SOURCE: "xg" } },
  { id: "npxg", label: "RATE_SOURCE=npxg", env: { SCORE_MAHER_RATE_SOURCE: "npxg" } },
  { id: "goals", label: "RATE_SOURCE=goals", env: { SCORE_MAHER_RATE_SOURCE: "goals" } },
  {
    id: "mix",
    label: "RATE_SOURCE=mix (0.7 npxG + 0.3 goals)",
    env: { SCORE_MAHER_RATE_SOURCE: "mix" },
  },
];
...
console.log("\n\nMaher rate-source comparison (rank by ROI, then Brier)");
console.log(
  [
    "id".padEnd(10),
    "n".padStart(6),
...
const outPath = resolve(projectRoot, "scripts/output/maher-rate-source-sweep.json");
writeFileSync(
  outPath,
