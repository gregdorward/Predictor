#!/usr/bin/env node
/**
 * Sweep SCORE_MAHER_RECENT_GAMES (recent window size for maher_recent).
 * Rank primarily by ROI, then Brier.
 *
 * Usage:
 *   NEXT_PUBLIC_EXPRESS_SERVER=http://localhost:5050/ \
 *     node scripts/sweep-maher-recent-games.mjs --from 2026-07-01 --to 2026-09-15
 */
...
const VARIANTS = [
  { id: "g5", label: "RECENT_GAMES=5 (baseline)", env: { SCORE_MAHER_RECENT_GAMES: "5" } },
  { id: "g8", label: "RECENT_GAMES=8", env: { SCORE_MAHER_RECENT_GAMES: "8" } },
  { id: "g10", label: "RECENT_GAMES=10", env: { SCORE_MAHER_RECENT_GAMES: "10" } },
];
...
console.log("\n\nMaher recent-games comparison (rank by ROI, then Brier)");
console.log(
  [
    "id".padEnd(8),
    "n".padStart(6),
...
const outPath = resolve(projectRoot, "scripts/output/maher-recent-games-sweep.json");
writeFileSync(
  outPath,
