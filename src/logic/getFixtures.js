...
export async function generateTables(a, leagueIdArray, allResults) {
  ...
  leagueArray.forEach(function (league) {
    ...
    if (GROUP_STAGE_LEAGUE_IDS.includes(currentLeagueId)) {
      const groupTables = transformGroupStageTables(league, currentLeagueId);

      groupTables.forEach(({ id, group, table: groupInstance }) => {
        tableArray.push({ id, group, table: groupInstance });

        const basicElements = groupInstance.map((item) => ({
...
export async function renderTable(index, results, id) {
  ...
  if (id !== 16504 && id !== 14236 && !GROUP_STAGE_LEAGUE_IDS.includes(id)) {
    ...
    const allGroupTeams = flattenGroupTables(groups);

    const allGroupTables = groups.map((group, groupIndex) => {
      return (
        <>
          <h3 className="GroupName">{group.group || group.name || group.round}</h3>
...
export async function generateFixtures(
  ...
  fixturesLoadPromise = (async () => {
    ...
    for (let i = 0; i < orderedLeagues.length; i++) {
      ...
      //       <div className="MissingPredictionsNotice">The leader at the end of the month will recieve a 3-month subscription to SoccerStatsHub. The winner will be based the highest remaining balance at the end of the month with each user starting on 50 units. Each tip will equate to 1 unit with the profit/loss being added to your running balance.</div>
      //       <Collapsable
      //         buttonText={"Filters"}
      //         className={"Filters2"}
      //         element={
